...
function result = StressEngine(xxi, xxj, z, f, alpha, beta, fsigma, rm)
   ...
   if (isnan(result)) %for some reason result sometimes gets to be NAN, not sure why this happens
       ...
       display('Warning: StressEngine encountered a NAN value where none was expected. Setting pixel to 0 instead.');
   end 
   if (result<0) %for some reason, not sure why this happens
       result = 0; %temproary fix is to set it zero then
       display('Warning: StressEngine encountered a negative value where none was expected. Setting pixel to 0 instead.');
   end 
   if (isreal(result)==0) %for some reason, not sure why this happens
       result = 0; %temproary fix is to set it zero then
       display('Warning: StressEngine encountered a complex value where none was expected. Setting pixel to 0 instead.');
   end 
end