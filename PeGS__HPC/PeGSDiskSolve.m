% Jonathan's Photoelastic Disk Solver (HPC version) 
%
...
%
% Last edit on 2017/03/07 by Jonathan Kollmer (jekollme@ncsu.edu)

%close all %housekeeping
%clear all %housekeeping
function PeGSDiskSolve(fileName)
...
%directory = '../1/Cycle01/';
%files = dir([directory, '*_preprocessing.mat']); 
...
%do we want to output a synthetic image based on the forces detected?
%if true we need to have the original image in the path. %TODO make it work
%without the original image, this is just a lazy workaround for determining
%image size.
saveSynthImage = true;
...
for frame = 1%:nFrames %loop over these frames 
    ...
    %fileName = files;%[directory,files(frame).name]; %which file/frame are we processing now ?
...
%                     template = template*3; %fine tunes the image, this should happen in preprocessing!
                    ...
                    for i=1:z
                            forces(i)=0.2*particle(n).f*particle(n).contactG2s(i)/cg2s;%initial guess for force
                    end
    ...
    if(saveSynthImage)
    	%realImageFiles = dir(['DSC*.JPG']); 
    	fileName = [fileName(1:end-18),'.JPG']; %which file/frame are we processing now ?
    	img = imread(fileName);
        bigSynthImg = zeros(size(img,1),size(img,2)); %make an empty image with the same size as the camera image
    	for n=1:length(pres) %for all particles
        	if (pres(n).z > 1 )
    	...
    	imwrite(bigSynthImg,[fileName(1:end-4),'_synth.jpg']);
